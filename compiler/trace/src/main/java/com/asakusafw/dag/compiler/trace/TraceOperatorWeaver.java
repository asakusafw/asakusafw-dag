...
import com.asakusafw.dag.runtime.trace.DefaultTraceOperator;
...
public class TraceOperatorWeaver implements OperatorRewriter {
    ...
    static final String KEY_COMPILER_OPTION = "tracepoint"; //$NON-NLS-1$

    static final AnnotationDescription TRACE_ANNOTATION;
    static {
        Map<String, ValueDescription> elements = Stream.of(Logging.class.getMethods())
                .filter(m -> m.getDefaultValue() != null)
                .collect(Collectors.toMap(
                        Method::getName,
                        m -> Descriptions.valueOf(m.getDefaultValue())));
    ...
    static void perform(OperatorGraph graph, ClassLoader classLoader, Collection<? extends TraceSetting> settings) {
        ...
        for (Operator operator : graph.getOperators(true)) {
            ...
            List<TraceSetting> scoped = map.getOrDefault(key, Collections.emptyList());
    ...
    private static Map<Key, List<TraceSetting>> parse(
        ...
        for (TraceSetting setting : Util.normalize(classLoader, settings)) {
            ...
            results.computeIfAbsent(key, k -> new ArrayList<>(1)).add(setting);
        }
        return results;
    }
