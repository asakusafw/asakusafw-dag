...
import com.asakusafw.dag.compiler.trace.testing.OperatorGraphBuilder;
...
public class TraceOperatorWeaverTest {
    ...
    @Test
    public void input() {
        OperatorGraph graph = new OperatorGraphBuilder()
                .operator("op", Ops.class, "update", c -> c
        ...
        Operator trace = Operators.getPredecessors(operator).stream().findFirst().get();
        checkTrace(trace);
    }
    ...
    public void output() {
        ...
        Operator trace = Operators.getSuccessors(operator).stream().findFirst().get();
        checkTrace(trace);
    }
    ...
    private Operator find(OperatorGraph graph, String name) {
        return graph.getOperators().stream()
                .filter(o -> o.getOperatorKind() == OperatorKind.USER)
                .map(UserOperator.class::cast)
                .filter(o -> o.getMethod().getName().equals(name))
                .findAny()
