...
/**
 * Test for {@link BasicConnectionPool}.
 */
public class BasicConnectionPoolTest extends JdbcDagTestRoot {
    ...
    public void simple() throws Exception {
        try (BasicConnectionPool pool = pool(1)) {
    ...
    public void cascade() throws Exception {
        try (BasicConnectionPool pool = pool(2)) {
            try (ConnectionPool.Handle handle = pool.acquire()) {
                insert(handle.getConnection(), 1, "1.0", "Hello1");
